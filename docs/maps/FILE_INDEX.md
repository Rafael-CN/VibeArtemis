...
Total: **213 arquivos**, **63.374 linhas**, **260 tipos**, **2488 métodos**.

## `app/src/main/java/com/limelight/`

| Arquivo | Linhas | Rank | Tipos | Símbolos-chave |
|---|--:|--:|---|---|
| [`Game.java`](../../app/src/main/java/com/limelight/Game.java) | 4349 | 15.1 | Game, GameMenuCallbacks | `onServiceConnected()`:242 `onServiceDisconnected()`:251 `run()`:319 `onCreate()`:342 `onCapturedPointer()`:531 `notifyCrash()`:657 `onDisplayAdded()`:1015 `onDisplayRemoved()`:1019 |
...
| [`AppView.java`](../../app/src/main/java/com/limelight/AppView.java) | 796 | 6.7 | AppView, AppObject | `onServiceConnected()`:94 `run()`:101 `run()`:145 `onServiceDisconnected()`:166 `onConfigurationChanged()`:172 `notifyComputerUpdated()`:202 `run()`:217 `run()`:231 |
...
| [`GameMenu.java`](../../app/src/main/java/com/limelight/GameMenu.java) | 349 | 12.8 | GameMenu, MenuOption | `onGlobalLayout()`:140 `showMenu()`:288 `hideMenu()`:337 `isMenuOpen()`:345 |
...
| [`StartExternalDisplayControlReceiver.java`](../../app/src/main/java/com/limelight/StartExternalDisplayControlReceiver.java) | 58 | 4.1 | StartExternalDisplayControlReceiver | `onReceive()`:25 |
...
| [`LimeLog.java`](../../app/src/main/java/com/limelight/LimeLog.java) | 26 | 88.9 | LimeLog | — |
...
| Arquivo | Linhas | Rank | Tipos | Símbolos-chave |
|---|--:|--:|---|---|
| [`PlatformBinding.java`](../../app/src/main/java/com/limelight/binding/PlatformBinding.java) | 15 | 4.8 | PlatformBinding | — |

## `app/src/main/java/com/limelight/binding/audio/`

| Arquivo | Linhas | Rank | Tipos | Símbolos-chave |
|---|--:|--:|---|---|
| [`AndroidAudioRenderer.java`](../../app/src/main/java/com/limelight/binding/audio/AndroidAudioRenderer.java) | 234 | 4.3 | AndroidAudioRenderer | `setup()`:68 `playDecodedAudio()`:189 `start()`:203 `stop()`:215 `cleanup()`:226 |
...
| [`InputCaptureProvider.java`](../../app/src/main/java/com/limelight/binding/input/capture/InputCaptureProvider.java) | 58 | 7.8 | InputCaptureProvider | `enableCapture()`:9 `disableCapture()`:13 `destroy()`:18 `isCapturingEnabled()`:20 `isCapturingActive()`:24 `showCursor()`:28 `hideCursor()`:32 `eventHasRelativeMouseAxes()`:36 |
...
| [`UsbDriverService.java`](../../app/src/main/java/com/limelight/binding/input/driver/UsbDriverService.java) | 367 | 3.7 | UsbDriverService, UsbEventReceiver, UsbDriverBinder, UsbDriverStateListener | `reportControllerState()`:46 `reportControllerMotion()`:55 `deviceRemoved()`:63 `deviceAdded()`:74 `onReceive()`:83 `run()`:99 `setListener()`:123 `setStateListener()`:134 |
...
| [`AbstractController.java`](../../app/src/main/java/com/limelight/binding/input/driver/AbstractController.java) | 88 | 3.4 | AbstractController | `getControllerId()`:22 `getVendorId()`:26 `getProductId()`:30 `getSupportedButtonFlags()`:34 `getCapabilities()`:38 `getType()`:42 `setButtonFlag()`:46 `reportInput()`:54 |
| [`UsbDriverListener.java`](../../app/src/main/java/com/limelight/binding/input/driver/UsbDriverListener.java) | 13 | 3.4 | UsbDriverListener | — |

## `app/src/main/java/com/limelight/binding/input/evdev/`

| Arquivo | Linhas | Rank | Tipos | Símbolos-chave |
|---|--:|--:|---|---|
| [`EvdevCaptureProviderShim.java`](../../app/src/main/java/com/limelight/binding/input/evdev/EvdevCaptureProviderShim.java) | 25 | 3.9 | EvdevCaptureProviderShim | — |
| [`EvdevListener.java`](../../app/src/main/java/com/limelight/binding/input/evdev/EvdevListener.java) | 16 | 4.2 | EvdevListener | — |
...
| [`KeyBoardLayoutController.java`](../../app/src/main/java/com/limelight/binding/input/virtual_controller/keyboard/KeyBoardLayoutController.java) | 380 | 4.4 | KeyBoardLayoutController, ViewCallbacks | `isModifierKeyPressed()`:86 `setViewCallbacks()`:112 `getHandler()`:116 `isKeyboardVisible()`:279 `hide()`:283 `hide()`:301 `show()`:305 `toggleVisibility()`:313 |
...
| [`ComputerManagerListener.java`](../../app/src/main/java/com/limelight/computers/ComputerManagerListener.java) | 8 | 3.7 | ComputerManagerListener | — |
...
| Arquivo | Linhas | Rank | Tipos | Símbolos-chave |
|---|--:|--:|---|---|
| [`NvConnection.java`](../../app/src/main/java/com/limelight/nvstream/NvConnection.java) | 629 | 7.9 | NvConnection | `stop()`:92 `quitAndLaunch()`:353 `run()`:391 `sendExecServerCmd()`:484 `sendKeyboardInput()`:537 `sendMouseScroll()`:543 `sendMouseHScroll()`:549 `sendMouseHighResScroll()`:555 |
...
| [`NvConnectionListener.java`](../../app/src/main/java/com/limelight/nvstream/NvConnectionListener.java) | 24 | 19.1 | NvConnectionListener | — |
...
| Arquivo | Linhas | Rank | Tipos | Símbolos-chave |
|---|--:|--:|---|---|
| [`AudioRenderer.java`](../../app/src/main/java/com/limelight/nvstream/av/audio/AudioRenderer.java) | 16 | 21.6 | AudioRenderer | — |

## `app/src/main/java/com/limelight/nvstream/av/video/`

| Arquivo | Linhas | Rank | Tipos | Símbolos-chave |
|---|--:|--:|---|---|
| [`VideoDecoderRenderer.java`](../../app/src/main/java/com/limelight/nvstream/av/video/VideoDecoderRenderer.java) | 22 | 19.9 | VideoDecoderRenderer | — |

## `app/src/main/java/com/limelight/nvstream/http/`

| Arquivo | Linhas | Rank | Tipos | Símbolos-chave |
|---|--:|--:|---|---|
| [`NvHTTP.java`](../../app/src/main/java/com/limelight/nvstream/http/NvHTTP.java) | 939 | 10.4 | NvHTTP | `chooseClientAlias()`:116 `chooseServerAlias()`:118 `getCertificateChain()`:120 `getClientAliases()`:123 `getPrivateKey()`:124 `getServerAliases()`:127 `getAcceptedIssuers()`:132 `checkClientTrusted()`:135 |
...
| [`ComputerDetails.java`](../../app/src/main/java/com/limelight/nvstream/http/ComputerDetails.java) | 240 | 24.9 | ComputerDetails, State, AddressTuple | `hashCode()`:37 `equals()`:42 `toString()`:51 `guessExternalPort()`:102 `update()`:123 `toString()`:172 |
...
| Arquivo | Linhas | Rank | Tipos | Símbolos-chave |
|---|--:|--:|---|---|
| [`ControllerPacket.java`](../../app/src/main/java/com/limelight/nvstream/input/ControllerPacket.java) | 27 | 18.4 | ControllerPacket | — |
| [`MouseButtonPacket.java`](../../app/src/main/java/com/limelight/nvstream/input/MouseButtonPacket.java) | 13 | 7.5 | MouseButtonPacket | — |
...
| Arquivo | Linhas | Rank | Tipos | Símbolos-chave |
|---|--:|--:|---|---|
| [`MoonBridge.java`](../../app/src/main/java/com/limelight/nvstream/jni/MoonBridge.java) | 427 | 55.9 | MoonBridge, AudioConfiguration | `getSurroundAudioInfo()`:165 `equals()`:170 `hashCode()`:180 `toInt()`:186 `startConnection()`:343 `stopConnection()`:353 `interruptConnection()`:355 `sendExecServerCmd()`:357 |
...
| Arquivo | Linhas | Rank | Tipos | Símbolos-chave |
|---|--:|--:|---|---|
| [`StreamSettings.java`](../../app/src/main/java/com/limelight/preferences/StreamSettings.java) | 1088 | 4.3 | StreamSettings, SettingsFragment | `onCreate()`:92 `onAttachedToWindow()`:106 `onConfigurationChanged()`:124 `onBackPressed()`:143 `getPrefs()`:174 `onCreateView()`:320 `onCreateView()`:326 `onCreatePreferences()`:331 |
| [`PreferenceConfiguration.java`](../../app/src/main/java/com/limelight/preferences/PreferenceConfiguration.java) | 1042 | 52.0 | PreferenceConfiguration, ScaleMode, FormatOption, AnalogStickForScrolling | — |
...
| Arquivo | Linhas | Rank | Tipos | Símbolos-chave |
|---|--:|--:|---|---|
| [`ProfilesManager.java`](../../app/src/main/java/com/limelight/profiles/ProfilesManager.java) | 266 | 34.0 | ProfilesManager, ProfilesData, ProfileChangeListener, OverlaySharedPreferences | `load()`:49 `save()`:102 `getProfiles()`:133 `add()`:137 `update()`:143 `delete()`:149 `setActive()`:158 `getActive()`:164 |
...
| [`ExternalControllerView.java`](../../app/src/main/java/com/limelight/ui/ExternalControllerView.java) | 94 | 4.4 | ExternalControllerView, InputCallbacks | `setInputCallbacks()`:16 `setCommitTextEnabled()`:20 `onKeyPreIme()`:34 `onCheckIsTextEditor()`:54 `onCreateInputConnection()`:59 `commitText()`:70 `deleteSurroundingText()`:78 |
| [`AdapterFragment.java`](../../app/src/main/java/com/limelight/ui/AdapterFragment.java) | 36 | 3.5 | AdapterFragment | `onAttach()`:18 `onCreateView()`:25 `onActivityCreated()`:31 |
| [`GameGestures.java`](../../app/src/main/java/com/limelight/ui/GameGestures.java) | 10 | 3.7 | GameGestures | — |
| [`AdapterFragmentCallbacks.java`](../../app/src/main/java/com/limelight/ui/AdapterFragmentCallbacks.java) | 9 | 3.5 | AdapterFragmentCallbacks | — |
...
| [`ExternalDisplayControlActivity.java`](../../app/src/main/java/com/limelight/utils/ExternalDisplayControlActivity.java) | 567 | 9.2 | ExternalDisplayControlActivity | `onCreate()`:114 `onResume()`:199 `onPause()`:207 `onDestroy()`:215 `onKeyboardControllerVisibilityChange()`:221 `onWindowFocusChanged()`:279 `onBackPressed()`:289 `onConfigurationChanged()`:316 |
| [`DeviceUtils.java`](../../app/src/main/java/com/limelight/utils/DeviceUtils.java) | 411 | 8.0 | DeviceUtils | — |
...
| [`ShortcutHelper.java`](../../app/src/main/java/com/limelight/utils/ShortcutHelper.java) | 315 | 4.0 | ShortcutHelper | `reportComputerShortcutUsed()`:97 `reportGameLaunched()`:105 `createAppViewShortcut()`:110 `createAppViewShortcutForOnlineHost()`:155 `createPinnedGameShortcut()`:164 `disableComputerShortcut()`:186 `disableAppShortcut()`:206 `enableAppShortcut()`:216 |
| [`UiHelper.java`](../../app/src/main/java/com/limelight/utils/UiHelper.java) | 296 | 12.7 | UiHelper | `onApplyWindowInsets()`:113 `onApplyWindowInsets()`:158 `run()`:199 `run()`:211 |
| [`ServerHelper.java`](../../app/src/main/java/com/limelight/utils/ServerHelper.java) | 275 | 5.2 | ServerHelper | `run()`:160 `run()`:198 |
...
| [`SpinnerDialog.java`](../../app/src/main/java/com/limelight/utils/SpinnerDialog.java) | 121 | 10.6 | SpinnerDialog | `run()`:62 `run()`:69 `onCancel()`:112 |
...
| [`Vector2d.java`](../../app/src/main/java/com/limelight/utils/Vector2d.java) | 48 | 3.4 | Vector2d | `initialize()`:14 `getMagnitude()`:20 `getNormalized()`:24 `scalarMultiply()`:28 `setX()`:32 `setY()`:36 `getX()`:40 `getY()`:44 |
| [`KeyConfigHelper.java`](../../app/src/main/java/com/limelight/utils/KeyConfigHelper.java) | 45 | 5.9 | KeyConfigHelper, ShortcutFile, Shortcut | — |
...
| [`simplejni.c`](../../app/src/main/jni/moonlight-core/simplejni.c) | 272 | — | — | — |
...
| [`TestLogSuppressor.java`](../../app/src/test/java/com/limelight/TestLogSuppressor.java) | 37 | 14.2 | TestLogSuppressor | `println()`:23 `print()`:30 |
